...
import { PortalProfessionals } from "../professionals";
...
function PortalInbox({ slug, portal, session, logout }: { slug: string; portal: PortalPublic; session: Session; logout: () => void }) {
  ...
  const featureOfView: Partial<Record<PortalView, PortalFeature>> = { inbox: "inbox", contacts: "contacts", calendar: "calendar", pipeline: "pipeline", reports: "reports", agents: "agents", api: "api", details: "details", professionals: "professionals" };
  // Agents, channels and the API also need their permission (admins only); `can()` already folds the function in
  // (channels: at least one channel type switched on).
  const viewOn = route.view === "settings" || (enabled(featureOfView[route.view] ?? "inbox") && (route.view !== "agents" || can("agents.manage")) && (route.view !== "channels" || can("channels.manage")) && (route.view !== "api" || can("api.manage")) && (route.view !== "details" || can("client.manage")));
  ...
  // The lead card beside the thread: its scope is this portal's routes and what this person may do.
  ...
  const leadScope = useMemo(() => portalLeadScope(slug, can, (number) => portalPath(urlBase, "inbox", number)), [slug, can, urlBase]);
  ...
  const inboxNodeRef = useCallback((node: HTMLDivElement | null) => { inboxRef.current = node; attachLead(node); }, [attachLead]);
  ...
  const clampListWidth = (width: number) => {
    // With the lead card open beside the thread, the thread keeps its minimum out of what is left of it.
  ...
  const refresh = useCallback(async () => {
    ...
    // An id of a thread another lead absorbed answers with the primary: follow it and move the address.
    if (conv.id !== openId) { selectedIdRef.current = conv.id; router.replace(portalPath(urlBase, "inbox", conv.number)); }
    setSelected((prev) => {
  ...
  useEffect(() => {
    ...
    api<Conversation>(`/portal/${slug}/conversations/number/${urlNumber}`)
      .then((detail) => {
        ...
        if (detail.number !== urlNumber) router.replace(portalPath(urlBase, "inbox", detail.number));
      })
  ...
  async function sendReaction(message: Message, emoji: string) {
    ...
    setError("");
    try {
      setSelected(await api<Conversation>(`/portal/${slug}/conversations/${selected.id}/messages/${message.id}/reaction`, { method: "POST", body: JSON.stringify({ emoji, ...(replyVia.multi ? { via_conversation_id: message.conversation_id ?? replyVia.via } : {}) }) }));
  ...
  async function replyWithTemplate(payload: TemplateSend) {
    if (!selected || replyChannel !== "whatsapp_cloud") return;
    applyThread(await api<Conversation>(`/portal/${slug}/conversations/${selected.id}/reply-template`, { method: "POST", body: JSON.stringify({ ...payload, ...replyVia.payload }) }));
  }
  ...
  const attachmentUrl = useCallback(
    (attachment: Attachment) => apiUrl(`/portal/${slug}/conversations/${owners.get(attachment.id) ?? selectedId}/attachments/${attachment.id}`),
    [slug, selectedId, owners],
  );
