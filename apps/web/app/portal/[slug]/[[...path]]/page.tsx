...
function PortalInbox({ slug, portal, session, logout }: { slug: string; portal: PortalPublic; session: Session; logout: () => void }) {
  ...
  const featureOfView: Partial<Record<PortalView, PortalFeature>> = { inbox: "inbox", contacts: "contacts", calendar: "calendar", pipeline: "pipeline", reports: "reports" };
  const viewOn = route.view === "settings" || enabled(featureOfView[route.view] ?? "inbox");
