...
import { FEATURE_OF_PERMISSION, hasFeature, type PortalFeature } from "@/lib/portal-features";
...
function PortalInbox({ slug, portal, session, logout }: { slug: string; portal: PortalPublic; session: Session; logout: () => void }) {
  ...
  const can = useCallback((key: string) => permissions.has(key) && (!(key in FEATURE_OF_PERMISSION) || enabled(FEATURE_OF_PERMISSION[key])), [permissions, enabled]);
  ...
  // Agents also needs the permission (admins only); `can()` already folds the function in.
  const viewOn = route.view === "settings" || (enabled(featureOfView[route.view] ?? "inbox") && (route.view !== "agents" || can("agents.manage")));
