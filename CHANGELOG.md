...
- **Coolify deployment.** `docker-compose.coolify.yml` (prebuilt images, no host ports, required
  ...
  `scripts/check-production-env.py` as a preflight, and the guide `docs/en|es/deploy-coolify.md`
  for a Hetzner server.

- **Public API docs and coverage map.** `docs/en/api.md` and `docs/es/api.md` guide an integrator through authentication (long-lived tokens, OAuth 2.0), every scope, all 47 `/api/v1` routes, the credential-management panel API, webhooks, idempotency and limits — both linked from the docs index. `docs/api-coverage.md` maps every panel screen to its API route (with the deliberately panel-only screens and their reasons), and `tests/test_api_coverage.py` enforces the map in both directions against the OpenAPI schema, so a screen without a route — or a route without its row — fails the suite.
- **Public API v1, the rest of the surface.** The versioned contract now covers agents (list, get, create, update, delete, plus the rendered prompt), the knowledge base (PDF upload with the panel's own best-effort indexing, list, delete, reindex, and Q&A pairs), a read-only unified channel list (QR and API WhatsApp numbers, Instagram/Messenger accounts, web chat, normalized to type/label/status), the thread oldest-first (visitor turns and `activity` notes alike), the reports (costs, replies, operations, all scoped to the client's numbers), and the calendar overview plus calendar-member management. Writes that create rows (agents, Q&A, calendar members) accept `Idempotency-Key` like the earlier v1 writes. Same scopes and client confinement as the panel routes; no migration.
- **Outbound webhooks.** Integrations forward inbox and pipeline events to their own URL: each integration holds subscriptions (a target URL plus the events it wants — `message.received`, `conversation.resolved`, `deal.moved`), and every matching event fans out into a delivery row per subscription. The background worker sends each delivery with an HMAC-SHA256 `X-Signature` over the raw body, climbing a 5/15/15/60-minute retry ladder before the row rests as `failed`; the delivery log, filterable by status, replays any row by hand. Secrets are shown once and stored encrypted, and deleting or revoking an integration stops its deliveries. Managed from the integration's expanded row (Settings → API integrations, or the client's API tab). Migration `0056` adds `webhook_subscriptions` and `webhook_deliveries`.
- **Idempotent writes.** Versioned API writes accept `Idempotency-Key`: the first request stores its answer for 24 hours, a retry with the same body replays it marked `"api_replay": true` instead of acting twice, another body under the same key is refused, and failures release the key so retrying is allowed. Keys are scoped per credential so integrations never replay each other. Migration `0055` adds `api_idempotency_keys`.
- **Public API v1.** The panel's resources with an integrator's contract under `/api/v1`: Kommo-shaped errors (`title`/`type`/`status`/`detail` plus `validation-errors`), `_links` on every resource, and `page`/`limit` pagination capped at 250 items. Covered: clients, contacts (list, get, create, update, search), conversations (list, get, operator reply under the panel's own human-hands rule), the pipeline board plus moving deals, and calendar events. Same bearer tokens and scopes as the panel routes; client-limited tokens stay confined because every route names its client.
- **OAuth 2.0 for third parties.** An integration doubles as an OAuth client: the agency registers its redirect URLs and takes a client secret for it, a signed-in person approves scopes on the `/oauth/authorize` consent screen, and the third party exchanges the single-use code for a one-hour access token plus a rotating 30-day refresh token (`POST /api/oauth/token`, `POST /api/oauth/revoke`). Granted subsets narrower than the integration travel with the grant, and refresh rotation retires the old pair. Migration `0054` adds the client identity to `api_integrations` and the grant linkage to `api_tokens`.
- **API credentials.** The panel's API answers to a token instead of a cookie session: an *integration* is one connection (a name, its scopes, and optionally a single client it is confined to) and its *long-lived tokens* are the secrets, issued with an expiry from one day to five years and shown once — only a SHA-256 digest is stored. Requests carry `Authorization: Bearer ol_…` or `X-API-Key: ol_…`. A token is held to its agency, its client and its scopes. Scoped today: clients, agents (with their knowledge and tools), the inbox, contact tags, teams, templates, the pipeline, the calendar, the channels and the reports. A route that declares no scope stays closed to tokens, so the reachable surface grows on purpose rather than by accident, and a route guarded only by `.read` scopes may only be a GET — a read-only credential is exactly that. Tokens also get their own 7-requests-per-second budget (429 with `retry_after`). Managing credentials needs the `integrations.manage` scope. Migration `0053` adds `api_integrations` and `api_tokens`.
...
- **The portal composer is a card.** A top row says what you do (**Chat**), *via* which
  ...
  **IA** button beside a **+** that opens upward with emoji, calendar, attach file and
  navigation. The bolt, calendar and navigation buttons have no action yet.
