...
| `details`, `professionals` | Off by default. `details` gives portal admins a Details screen to edit the business name, industry, time zone and logo (never the portal settings or the active flag). `professionals` lists the client's staff, each with a weekly working schedule; portal admins can add and edit them, other users see the list. It only stores schedules for now: the agent does not book by them yet. |
