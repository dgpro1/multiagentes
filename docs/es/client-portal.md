...
| `details`, `professionals` | Apagadas de entrada. `details` da a los administradores del portal una pantalla de Detalles para editar el nombre del negocio, la industria, la zona horaria y el logo (nunca los ajustes del portal ni el estado activo). `professionals` lista al personal del cliente, cada uno con su horario semanal; los administradores del portal pueden agregarlos y editarlos, los demás solo ven la lista. Por ahora solo guarda horarios: el agente todavía no agenda con ellos. |
